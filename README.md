...
Each tagged release ships two artifacts, both produced from the same commit. The
`eth-ssz-specs` package is published to PyPI:

```bash
pip install eth-ssz-specs   # or: uv add eth-ssz-specs
```

The conformance vectors are attached to each entry on the
[releases page](https://github.com/leanEthereum/ssz-specs/releases) as a deterministic
tarball plus a checksum. Pick a tag, then download, verify, and extract:
...
tar -xzf "ssz-test-vectors-$TAG.tar.gz"   # extracts a single fixtures/ directory
```

The vectors attached to tag `vX.Y.Z` are guaranteed to have been generated by exactly
that commit's spec code: the release workflow fills, packages, and attaches them in the
same run that creates the tag, and creates no tag if any step fails.
